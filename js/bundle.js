...
function calc() {
  ...
  let sex, height, weight, age, ratio; // Проверяем наличие записей в localstorage
      ...
      if (item.tagName == "INPUT") {
        ...
        item.addEventListener('click', e => {
          ...
          if (e.target.hasAttribute('data-ratio')) {
            ...
            items.forEach(item => {
              // Удаляем классы активности у всех элементов
...
function cards() {
  ...
  class CardElement {
    constructor(title, desciption, price, currency, img, parent) {
      this.title = title;
    ...
    render() {
      const block = document.createElement('div'); //Если в конструктор класса передано больше классов, то добавляем их на карточку
  ...
  (0,_service_services__WEBPACK_IMPORTED_MODULE_0__.getData)('http://localhost:3000/menu').then(data => {
    data.forEach(_ref => {
      let {
        img,
        title,
      ...
      new CardElement(title, descr, price, 'грн', img, '.menu__field .container', 'menu__item').render();
    });
  }).catch(data => {
...
function form(formSelector) {
  ...
  function sendDataForm(form) {
    form.addEventListener('submit', e => {
      ...
      (0,_service_services__WEBPACK_IMPORTED_MODULE_1__.postData)('http://localhost:3000/requests', json).then(data => {
        console.log(data);
...
function simpleSlider(_ref) {
  let {
    ...
    // Класс слайдов
    slideToScroll,
    // Количество слайдов для скролла (только для стрелок)
    slidePrev,
    // Класс навигации (Предыдущий слайд)
    slideNext,
    // Класс навигации (Следующий слайд)
    field,
    // Контейнер для слайдов (track)
    transform,
    // Смещение слайдов через transform (true/false)
  ...
  let slideIndex = 1,
      ...
      offset = 0; // Если свойство transform = true, смещение слайдов будет реализовано через translate
  ...
  if (dots) {
    ...
    sliderWrapper.append(dotsContainer); // Переключаем слайд по клику на элемент
  ...
  function changeSlide(index) {
    slideIndex = index; // Цикличное переключение слайдов
    ...
    sliderWrapper.setAttribute('data-current-slide', slideIndex); // Устанавливаем класс активности для точек
...
function timer(selector, timeToEnd) {
  ...
  function GetTimeRemaining(endtime) {
    ...
    const time = Date.parse(endtime) - Date.parse(new Date()); // Устанавливаем значение при окончании таймера
...
const getData = async url => {
  let res;
  await fetch(url).then(data => data.json()).catch(data => {
    console.log(data);
  }).finally(data => {
    res = data;
  });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2022-07-20');
});
})();
